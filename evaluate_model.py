...
def evaluate_model(label_directory, output_directory):
    ...
    binary_outputs, scalar_outputs = load_classifier_outputs(
        output_files, classes)

    # Evaluate the model by comparing the labels and outputs.
    print('Evaluating model...')

    print('- AUROC and AUPRC...')
    auroc, auprc, auroc_classes, auprc_classes = compute_auc(
        labels, scalar_outputs)
    ...
    print('Done.')
    # Return the results.
    return classes, auroc, auprc, auroc_classes, auprc_classes, accuracy, f_measure, f_measure_classes
...
def find_challenge_files(label_directory, output_directory):
    ...
    for label_file in sorted(os.listdir(label_directory)):
        label_file_path = os.path.join(
            label_directory, label_file)  # Full path for label file
...
def load_classifier_outputs(output_files, classes):
    ...
    # Iterate over the recordings.
    for i in range(num_recordings):
        recording_id, recording_classes, recording_binary_outputs, recording_scalar_outputs = load_outputs(
            output_files[i])

        # Allow for equivalent classes and sanitize classifier outputs.
        recording_classes = [set(entry.split('|'))
                             for entry in recording_classes]
        recording_binary_outputs = [1 if entry in (
            '1', 'True', 'true', 'T', 't') else 0 for entry in recording_binary_outputs]
        recording_scalar_outputs = [float(entry) if is_finite_number(
            entry) else 0 for entry in recording_scalar_outputs]
        ...
        for j, x in enumerate(classes):
            ...
            if binary_values:
                # Define a class as positive if any of the equivalent classes is positive.
                binary_outputs[i, j] = any(binary_values)
            if scalar_values:
                # Define the scalar value of a class as the mean value of the scalar values across equivalent classes.
...
if __name__ == '__main__':
    classes, auroc, auprc, auroc_classes, auprc_classes, accuracy, f_measure, f_measure_classes, challenge_metric = evaluate_model(
        sys.argv[1], sys.argv[2])
    output_string = 'AUROC,AUPRC,Accuracy,F-measure,Challenge metric\n{:.3f},{:.3f},{:.3f},{:.3f},{:.3f}'.format(
        auroc, auprc, accuracy, f_measure, challenge_metric)
